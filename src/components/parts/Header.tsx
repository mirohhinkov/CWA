    ...
    <nav className="container menu_justify">
      <MenuItems>
        <ul>
          ...
          {!context.login && (<li><Link className={context.activeMenu === 'login' ? 'active' : ''} to="/login">Login</Link></li>)}
          {context.login && (<li><Link className={context.activeMenu === 'login' ? 'active' : ''} to="/login">Logout</Link></li>)}
        </ul>
      </MenuItems>
    </nav>
  </header>
